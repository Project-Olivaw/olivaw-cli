...
///
/// This is the *only* angle-wrapping function in the module; every [`Pose2`]
...
mod tests {
    ...
    /// Plain-assert stand-in for `approx`, keeping the component dep-free.
    ...
    fn normalize_angle_boundaries() {
        let eps = 1e-12;
        assert_close(normalize_angle(0.0), 0.0, eps);
        // +π and -π both map to +π (half-open interval (-π, π]).
        assert_close(normalize_angle(PI), PI, eps);
        assert_close(normalize_angle(-PI), PI, eps);
        assert_close(normalize_angle(2.0 * PI), 0.0, eps);
        assert_close(normalize_angle(-2.0 * PI), 0.0, eps);
        assert_close(normalize_angle(3.0 * PI), PI, eps);
        assert_close(normalize_angle(-3.0 * PI), PI, eps);
    }
    ...
    fn normalize_angle_near_boundaries() {
        let eps = 1e-9;
        assert_close(normalize_angle(PI - eps), PI - eps, 1e-12);
        // Just past +π wraps to just above -π.
        assert_close(normalize_angle(PI + eps), -PI + eps, 1e-9);
        assert_close(normalize_angle(-PI + eps), -PI + eps, 1e-12);
        assert_close(normalize_angle(10.0 * PI + 0.5), 0.5, 1e-9);
    }
    ...
    fn compose_hand_computed() {
        ...
        let c = a.compose(&b);
        assert_close(c.x, 1.0, 1e-12);
        assert_close(c.y, 1.0, 1e-12);
        assert_close(c.theta, PI / 2.0, 1e-12);
    }
    ...
    fn inverse_of_pi_heading_composes_to_identity() {
        ...
        assert_close(id.x, 0.0, 1e-12);
        assert_close(id.y, 0.0, 1e-12);
        assert_close(normalize_angle(id.theta), 0.0, 1e-12);
    }
    ...
    fn transform_point_matches_isometry() {
        ...
        assert_close(ours.x, theirs.x, 1e-12);
        assert_close(ours.y, theirs.y, 1e-12);
    }
    ...
    fn between_recovers_relative_pose() {
        ...
        assert_close(recovered.x, rel.x, 1e-12);
        assert_close(recovered.y, rel.y, 1e-12);
        assert_close(recovered.theta, rel.theta, 1e-12);
    }
}
